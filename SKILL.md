...
| 04 内容生成层 | 根据大纲生成口播稿、公众号长文和配图 Prompt。 | 口播脚本（`voice-script-generator`）、大纲扩写（`outline-expander`）、生成配图（`content-image-gen`） |
...
- 正文或口播稿完成后，可自动进入内容审稿；审稿完成后只给审稿结论和修改建议。
- 公众号工作台只在用户明确要求预览、排版或推送草稿时启动。
